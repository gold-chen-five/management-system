...
const ItemInfo:React.FC<{item: ApplicationListInterface | undefined, setItemClick:React.Dispatch<React.SetStateAction<boolean>>}> = ({item,setItemClick}) => {
  ...
  return (
    <div className='w-full h-full flex flex-col items-center pt-10 space-y-4 sm:pt-28'>
      <div className='flex items-center bg-[#27272A] w-1/2 h-10 border border-[#3F3F46] rounded-lg outline-none text-[#D4D4D2] pl-4 pr-4 font-fontJapan font-semibold sm:w-4/5'>
        <p>項次: {item?.line}</p>
      </div>
      <div className='flex items-center bg-[#27272A] w-1/2 h-10 border border-[#3F3F46] rounded-lg outline-none text-[#D4D4D2] pl-4 pr-4 font-fontJapan font-semibold sm:w-4/5'>
        <p>項目: {item?.item}</p>
